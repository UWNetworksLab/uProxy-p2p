/**
 * The roster keeps track of delta updates to the buddy list.
 */
function Roster(agent, daemon) {
  this.agent = agent;
  this.daemon = daemon;
  this.jids = {};
...
Roster.prototype.getFriends = function() {
  ...
  for (var i in this.jids) {
    ...
    if (i.indexOf("/" + this.agent) > 0 && i != this.daemon.client.jid.toString()) {
      friends.push(i);
    }
  }
  return friends;
...
function XmppDaemon(agent, credentials) {
  this.roster = new Roster(agent, this);
  this.client = new XMPP.Client({
    xmlns:'jabber:client',
    jid: credentials.email + "/" + agent,
