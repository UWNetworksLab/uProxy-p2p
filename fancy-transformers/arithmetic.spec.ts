...
describe('Arithmetic coding and decoding - short inputs', function() {
  it('Encode(0.1.2.3)==ca.0.1.2.3.0.0.0.8', function() {
    var plain = arraybuffers.hexStringToArrayBuffer("0.1.2.3");
    var target = arraybuffers.hexStringToArrayBuffer("ca.0.1.2.3.0.0.0.8");
    var encoder = new arithmetic.Encoder(makeUniformProbabilities());
    var result = encoder.encode(plain);
    expect(arraybuffers.byteEquality(result, target)).toBe(true);
  });
  it('Decode(ca.0.1.2.3.0.0.0.8)==0.1.2.3', function() {
    var encoded = arraybuffers.hexStringToArrayBuffer("ca.0.1.2.3.0.0.0.8");
    var target = arraybuffers.hexStringToArrayBuffer("0.1.2.3");
