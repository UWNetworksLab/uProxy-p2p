...
describe('Core', () => {
  ...
  network['login'] = (loginType :uproxy_core_api.LoginType) => { return Promise.resolve<void>() };
  network['myInstance'] =
  ...
  it('login fails for invalid network', (done) => {
    core.login({network: 'nothing', loginType: uproxy_core_api.LoginType.INITIAL}).catch(() => {
      done();
    });
  });
  ...
  it('login continues to call login on correct network', (done) => {
    ...
    core.login({network: 'mockNetwork', loginType: uproxy_core_api.LoginType.INITIAL});
    ...
    core.login({network: 'mockNetwork', loginType: uproxy_core_api.LoginType.INITIAL}).then(() => {
