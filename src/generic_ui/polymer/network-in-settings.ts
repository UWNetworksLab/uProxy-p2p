...
Polymer({
  ...
  logout: function() {
    ...
    var confirmationMessage = util.getLogoutConfirmationMessage(isGettingForThisNetwork, isSharingForThisNetwork);

    var confirmLogout = Promise.resolve<void>();
    if (confirmationMessage) {
      confirmLogout = this.$.state.openDialog(util.getConfirmationObject('', confirmationMessage));
    }
