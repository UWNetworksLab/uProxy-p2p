...
function _updateInstanceIdsOnChange(prior, current) {
  ...
  for (var client in current.clients) {
    ...
    if (state.roster[current.userId] == undefined ||
        state.roster[current.userId]['instanceId'] == undefined) {
      if (state.roster[current.userId] == undefined) {
        state.roster[current.userId] = { "instanceId": "(requested)" };
      } else {
        state.roster[current.userId].instanceId = '(requested)';
      }
      ...
      if (_isMessageableUproxy(cur) && ((prev == null) || !_isMessageableUproxy(prev))) {
        log.debug('_updateInstanceIdsOnChange: deciding to message ' + current.userId);
...
function _dispatchInstanceIdQuery(user, client) {
  if (client['network'] == undefined || (client.network != 'loopback' && client.network != 'manual')) {
    ...
    identity.sendMessage(client.userId,
                         JSON.stringify({ message: 'request-instance-id', refclient: client }));
  }
}
